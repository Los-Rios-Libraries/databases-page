...
function writeDBInfo($db, $url) {
    ...
    $name = $db -> name;
    if (preg_match('/Bloomsbury|Coronavirus|Country|Dailies|(Digital|National) Thea|Flipster|Kanopy|Opposing|CollegeSource|Ethnologue|Health Reference|ProQuest|Global Env|NewsWatch|Swank|InfobaseBooks/', $name) === 1) {
        $searchButton = '';
    }
    $output .= $searchButton;

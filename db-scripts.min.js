...
$(function(){var c=$("#main .db-name").length,d=$("#show-db-no");70>c?(dbNoS=c.toString(),$("#db-no").text(dbNoS),1===c&&d.text(d.text().replace("Databases","Database")),d.attr("class","show")):d.attr("class","hidden").attr("aria-hidden","true");$(".db-name").each(function(){$(this).text($(this).text().replace("Acls","ACLS").replace("Cinahl","CINAHL").replace("Cq","CQ").replace("Ebook Coll","eBook Coll").replace("Eric","ERIC").replace("Medline","MEDLINE"))})});
$(function(){var c=$("#main-nav"),d=$("#nav-label");781>$(window).width()&&c.addClass("hidden");d.on("click",function(){c.toggleClass("hidden")})});$(function(){$(".category").each(function(){var c=$(this);c.removeClass("active hidden");c.find("li").hasClass("active")?c.addClass("active"):c.addClass("hidden")});checkZeros()});$(".alpha").each(function(){0===$(this).find("li").length&&$(this).addClass("hidden")});$(function(){checkZeros()});
function checkZeros(){if(!$(".problem-description").length&&($("#zero-notice").remove(),0===$("#main .active").length)){var c=$("#main");c.append('<div id="zero-notice">There are no databases fitting the criteria you indicated. Sorry!</div>');c.append($("#multi-search"));$("#dbpage-query").addClass("form-emphasis");$("#form-remainder").removeClass("hidden")}}$(function(){$("dt").prev("dd").addClass("last-dd")});$("#dbpage-query").on("focus",function(){showSearch($("#form-remainder"),$(this))});
function showSearch(c,d){d.addClass("form-emphasis");c.slideDown().removeClass("hidden");$("#form-closer").on("click",function(f){f.preventDefault();hideSearch(c,d)});$("#dbpage-query").autocomplete({source:function(c,d){var h=$.ui.autocomplete.filter(dbNames,c.term);d(h.slice(0,5))},select:function(c,d){d.item&&$(this).val(d.item.value);$("#multi-search").submit()}})}function hideSearch(c,d){c.slideUp().addClass("hidden");d.removeClass("form-emphasis");d.on("click",function(){showSearch(c,d)})}
$("#main-nav li a").each(function(){var c=$("#title-cat").text(),d=$("#title-alpha").text().toLowerCase(),f=$(this),g=f.text();c!==g&&d!==g||f.addClass("current")});$(".format-links").each(function(){var c=$(this);c.find("a").text()===$("#title-format").text()&&c.addClass("current")});

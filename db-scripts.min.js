...
$("#multi-search").on("submit",function(){var c=$('input[name="search-type"]:checked').val(),d=$("#dbpage-query").val();ga("send","event","search","submit - "+c,d)});function setCookie(c,d,f){var g=new Date;g.setTime(g.getTime()+864E5*f);f="expires="+g.toUTCString();document.cookie=c+"="+d+"; "+f}$(".headnav a, #choose-library button").on("click",function(){var c=$(this).text().toLowerCase();setCookie("homeLibrary",c,10)});
...
function homeLibEls(c){var d;switch(c){case "arc":d="amerriv";break;case "scc":d="sacram";break;case "crc":d="cosum";break;case "flc":d="ns015092";break;default:d=""}$("#"+c+"-link").addClass("homelib");$.get("help/"+c+".php",function(c){$("#library-help-content").html(c)},"html");var f=c.toUpperCase();$("#library-help h2").html("From the "+f+" Library");$("#pubfinder a").attr("href","http://search.ebscohost.com/login.aspx?authtype=ip,guest&direct=true&custid="+d+"&db=edspub&groupid=main&profile=eds&plp=1");
setTimeout(function(){$("#library-help-content a").on("click",function(){var d=$(this);d.text().length?d=d.text():d.find("img").length?(b=d.find("img"),d=b.attr("alt").length?b.attr("alt"):""):d="";ga("send","event",""+c+" box","click",d)})},800)}
function checkCookies(c){"homeLibrary"===c?(c=getCookie(c),"arc"===c?homeLibEls("arc"):"scc"===c?homeLibEls("scc"):"crc"===c?homeLibEls("crc"):"flc"===c?homeLibEls("flc"):$.get("help/unknown.php",function(c){$("#library-help-content").html(c)},"html"),console.log(c)):"newWindowLinks"===c&&"yes"===getCookie(c)&&($("#newwin-check").prop("checked",!0),$(".db-name, .headnav a, #pubfinder a").attr("target","_blank"),setTimeout(function(){$("#library-help-content a").attr("target","_blank")},1E3))}
...
$("#choose-library button").on("click",function(){var c=$(this).text();homeLibEls(c)});var currentURL=location.href;currentURL.match("college=")&&(currentURL=currentURL.replace(/(\?|&)college.*/,""),replaceURL=currentURL.replace(/.*losrios.edu\//,""),history.replaceState(currentURL,document.title,currentURL));
...
c.addClass("hidden"),c.after(d),c.next(l).fadeIn(),c.next(l).find(":text").focus())});function hideZ(){$(".zopim").addClass("hidden")}function showZ(){$(".zopim").removeClass("hidden")};
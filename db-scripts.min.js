...
function homeLibEls(c){var d,f;switch(c){case "arc":d="amerriv";f="240535";break;case "scc":d="sacram";f="106093";break;case "crc":d="cosum";f="237206";break;case "flc":d="ns015092";f="237742";break;default:d=""}$("#"+c+"-link").addClass("homelib");$.get("help/"+c+".php",function(c){$("#library-help-content").html(c)},"html");var g=c.toUpperCase();$("#library-help h2").html("From the "+g+" Library");$("#pubfinder a").attr("href","http://search.ebscohost.com/login.aspx?authtype=ip,guest&direct=true&custid="+
d+"&db=edspub&groupid=main&profile=eds&plp=1");$(".db-name:contains(Films on Demand)").attr("href","http://0-fod.infobase.com.lasiii.losrios.edu/PortalPlayLists.aspx?wid="+f);setTimeout(function(){$("#library-help-content a").on("click",function(){var d=$(this);d.text().length?d=d.text():d.find("img").length?(b=d.find("img"),d=b.attr("alt").length?b.attr("alt"):""):d="";ga("send","event",""+c+" box","click",d)})},800)}
...
$("#choose-library button").on("click",function(){$('script[src*="libraryh3lp"]').remove();$(".zopim").remove();var c=$(this).text();homeLibEls(c)});var currentURL=location.href;currentURL.match("college=")&&(currentURL=currentURL.replace(/(\?|&)college.*/,""),replaceURL=currentURL.replace(/.*losrios.edu\//,""),history.replaceState(currentURL,document.title,currentURL));
...
c.addClass("hidden"),c.after(d),c.next(l).fadeIn(),c.next(l).find(":text").focus())});$("#ga-opt-out").on("click",function(c){c.preventDefault();var d=screen.availHeight;c=.7*d;var d=.15*d,f=screen.width/2-350,g=$(this).attr("href");window.open(g,"optOut","height="+c+", width=700, menubar=no, left="+f+", top="+d+"").focus()});
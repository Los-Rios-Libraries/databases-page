...
location.href="index.php?az&query="+encodeURIComponent(a)):(a="http://0-search.ebscohost.com.lasiii.losrios.edu/login.aspx?authtype=ip,uid&direct=true&profile=eds&bquery="+encodeURIComponent(a)+"&site=eds-live&scope=site","yes"===getCookie("newWindowLinks")?window.open(a):location.href=a)}
...
$(".open-db-search").on("click",function(){var a=$(this),c="";"yes"===getCookie("newWindowLinks")&&(c=' target="_blank" ');var d=a.nextAll("h3").find(".db-name"),e=d.text(),f=e.toLowerCase();f=f.replace(/ /g,"-");var h=a.nextAll("h3").find(".vendor").text().toLowerCase();h=h.replace(/[\(\):]/g,"");h=h.replace(/ /g,"-");var k=document.getElementById(f);$(".open-db-search").removeClass("hidden");var g=$(".search-one-db");k?(a.addClass("hidden"),g.hide(),a.next(g).fadeIn(),document.getElementById(f).focus()):
(g.hide(),d=d.attr("href"),k="",-1<d.indexOf("ehost")&&(k='<input type="hidden" name="ehost" value="'+d.split(/[= ]+/).pop()+'">'),c="<form onsubmit=\"ga('send', 'event', 'quick search', 'submit', '"+e+'\');" class="search-one-db" method="post" action="search-db.php" '+c+'><label class="search-one-db-l" for="'+f+'">Search '+e+'</label><input name="query" id="'+f+'" type="text"><input type="hidden" name="vendor" value="'+h+'"><input type="hidden" name="db-name" value="'+f+'">'+k+'<button class="search-btn" type="submit"><img height="16" width="16" src="search.png" alt="search"></button></form>',
a.addClass("hidden"),a.after(c),a.next(g).fadeIn(),a.next(g).find(":text").focus())});$("#ga-opt-out").on("click",function(a){a.preventDefault();var c=screen.availHeight;a=.7*c;c*=.15;var d=screen.width/2-350,e=$(this).attr("href");window.open(e,"optOut","height="+a+", width=700, menubar=no, left="+d+", top="+c+"").focus()});$("#remove-proxy").on("click",function(){removeProxy();setCookie("dbProxy","removed",null)});var proxySet=getCookie("dbProxy");"removed"===proxySet&&removeProxy();

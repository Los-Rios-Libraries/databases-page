...
$(function(){defaultAuto($("#dbpage-query"));$(".db-name").each(function(){for(var c=$(this).html().split(" "),a=/^Ahfs|^Apa$|^Ebsco|Cinahl|Cq|Eric|^Mas$|Medline|^Sage/,d=0;d<c.length;d++)a.test(c[d])?c[d]=c[d].toUpperCase():"Ebook"===c[d]&&(c[d]="eBook");c=c.join(" ");$(this).html(c)});var c=$("#main-nav"),f=$("#nav-label");781>$(window).width()&&c.addClass("hidden");f.on("click",function(){c.toggleClass("hidden")});$(".category").each(function(){var c=$(this);c.removeClass("active hidden");var a=
c.find("li");a.hasClass("active")&&a.is(":visible")?c.addClass("active"):c.addClass("hidden")});checkZeros();$("dt").prev("dd").addClass("last-dd");$("#trial-dbs .db-desc").each(function(){var c=$(this),a=c.html(),d=$.trim(a).substring(0,60).split(" ").slice(0,-1).join(" ")+'... <button class="trial-desc-exp" type="button">more</button>';c.html(d);c.show();$(".trial-desc-exp").on("click",function(){$(this).parent().html(a)})})});

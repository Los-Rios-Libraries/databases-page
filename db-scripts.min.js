/*
 ...
 http://jquery.org/license
*/
(function(c){"function"===typeof define&&define.amd?define(["jquery"],c):c(jQuery)})(function(c){c.ui=c.ui||{};c.ui.version="1.12.1";var m=0,q=Array.prototype.slice;c.cleanData=function(a){return function(d){var e,f,h;for(h=0;null!=(f=d[h]);h++)try{(e=c._data(f,"events"))&&e.remove&&c(f).triggerHandler("remove")}catch(k){}a(d)}}(c.cleanData);c.widget=function(a,d,e){var f={},h=a.split(".")[0];a=a.split(".")[1];var k=h+"-"+a;e||(e=d,d=c.Widget);c.isArray(e)&&(e=c.extend.apply(null,[{}].concat(e)));
c.expr[":"][k.toLowerCase()]=function(B){return!!c.data(B,k)};c[h]=c[h]||{};var p=c[h][a];var r=c[h][a]=function(B,F){if(!this._createWidget)return new r(B,F);arguments.length&&this._createWidget(B,F)};c.extend(r,p,{version:e.version,_proto:c.extend({},e),_childConstructors:[]});var w=new d;w.options=c.widget.extend({},w.options);c.each(e,function(B,F){c.isFunction(F)?f[B]=function(){function g(){return d.prototype[B].apply(this,arguments)}function l(n){return d.prototype[B].apply(this,n)}return function(){var n=
this._super,t=this._superApply;this._super=g;this._superApply=l;var v=F.apply(this,arguments);this._super=n;this._superApply=t;return v}}():f[B]=F});r.prototype=c.widget.extend(w,{widgetEventPrefix:p?w.widgetEventPrefix||a:a},f,{constructor:r,namespace:h,widgetName:a,widgetFullName:k});p?(c.each(p._childConstructors,function(B,F){var g=F.prototype;c.widget(g.namespace+"."+g.widgetName,r,F._proto)}),delete p._childConstructors):d._childConstructors.push(r);c.widget.bridge(a,r);return r};c.widget.extend=
function(a){for(var d=q.call(arguments,1),e=0,f=d.length,h,k;e<f;e++)for(h in d[e])k=d[e][h],d[e].hasOwnProperty(h)&&void 0!==k&&(c.isPlainObject(k)?a[h]=c.isPlainObject(a[h])?c.widget.extend({},a[h],k):c.widget.extend({},k):a[h]=k);return a};c.widget.bridge=function(a,d){var e=d.prototype.widgetFullName||a;c.fn[a]=function(f){var h="string"===typeof f,k=q.call(arguments,1),p=this;h?this.length||"instance"!==f?this.each(function(){var r=c.data(this,e);if("instance"===f)return p=r,!1;if(!r)return c.error("cannot call methods on "+
a+" prior to initialization; attempted to call method '"+f+"'");if(!c.isFunction(r[f])||"_"===f.charAt(0))return c.error("no such method '"+f+"' for "+a+" widget instance");var w=r[f].apply(r,k);if(w!==r&&void 0!==w)return p=w&&w.jquery?p.pushStack(w.get()):w,!1}):p=void 0:(k.length&&(f=c.widget.extend.apply(null,[f].concat(k))),this.each(function(){var r=c.data(this,e);r?(r.option(f||{}),r._init&&r._init()):c.data(this,e,new d(f,this))}));return p}};c.Widget=function(){};c.Widget._childConstructors=
...
_classes:function(a){function d(h,k){var p;for(p=0;p<h.length;p++){var r=f.classesElementLookup[h[p]]||c();r=a.add?c(c.unique(r.get().concat(a.element.get()))):c(r.not(a.element).get());f.classesElementLookup[h[p]]=r;e.push(h[p]);k&&a.classes[h[p]]&&e.push(a.classes[h[p]])}}var e=[],f=this;a=c.extend({element:this.element,classes:this.options.classes||{}},a);this._on(a.element,{remove:"_untrackClassesElement"});a.keys&&d(a.keys.match(/\S+/g)||[],!0);a.extra&&d(a.extra.match(/\S+/g)||[]);return e.join(" ")},
...
_on:function(a,d,e){var f,h=this;"boolean"!==typeof a&&(e=d,d=a,a=!1);e?(d=f=c(d),this.bindings=this.bindings.add(d)):(e=d,d=this.element,f=this.widget());c.each(e,function(k,p){function r(){if(a||!0!==h.options.disabled&&!c(this).hasClass("ui-state-disabled"))return("string"===typeof p?h[p]:p).apply(h,arguments)}"string"!==typeof p&&(r.guid=p.guid=p.guid||r.guid||c.guid++);var w=k.match(/^([\w:-]*)\s*(.*)$/),B=w[1]+h.eventNamespace;if(w=w[2])f.on(B,w,r);else d.on(B,r)})},_off:function(a,d){d=(d||
...
p.test(y[0])?["center"].concat(y):["center","center"]);y[0]=k.test(y[0])?y[0]:"center";y[1]=p.test(y[1])?y[1]:"center";var G=r.exec(y[0]);var H=r.exec(y[1]);x[this]=[G?G[0]:0,H?H[0]:0];g[this]=[w.exec(y[0])[0],w.exec(y[1])[0]]});1===v.length&&(v[1]=v[0]);"right"===g.at[0]?E.left+=D:"center"===g.at[0]&&(E.left+=D/2);"bottom"===g.at[1]?E.top+=z:"center"===g.at[1]&&(E.top+=z/2);var L=a(x.at,D,z);E.left+=L[0];E.top+=L[1];return this.each(function(){var y,G=c(this),H=G.outerWidth(),I=G.outerHeight(),R=
...
var f=e.filter(":not(.ui-menu)").hide().attr({role:this.options.role,"aria-hidden":"true","aria-expanded":"false"}).each(function(){var k=c(this),p=k.prev(),r=c("<span>").data("ui-menu-submenu-caret",!0);a._addClass(r,"ui-menu-icon","ui-icon "+d);p.attr("aria-haspopup","true").prepend(r);k.attr("aria-labelledby",p.attr("id"))});this._addClass(f,"ui-menu","ui-widget ui-widget-content ui-front");f=e.add(this.element).find(this.options.items);f.not(".ui-menu-item").each(function(){var k=c(this);a._isDivider(k)&&
...
f?"val":"text"];this.isNewMenu=!0;this._addClass("ui-autocomplete-input");this.element.attr("autocomplete","off");this._on(this.element,{keydown:function(k){if(this.element.prop("readOnly"))d=e=a=!0;else{d=e=a=!1;var p=c.ui.keyCode;switch(k.keyCode){case p.PAGE_UP:a=!0;this._move("previousPage",k);break;case p.PAGE_DOWN:a=!0;this._move("nextPage",k);break;case p.UP:a=!0;this._keyEvent("previous",k);break;case p.DOWN:a=!0;this._keyEvent("next",k);break;case p.ENTER:this.menu.active&&(a=!0,k.preventDefault(),
this.menu.select(k));break;case p.TAB:this.menu.active&&this.menu.select(k);break;case p.ESCAPE:this.menu.element.is(":visible")&&(this.isMultiLine||this._value(this.term),this.close(k),k.preventDefault());break;default:d=!0,this._searchTimeout(k)}}},keypress:function(k){if(a)a=!1,this.isMultiLine&&!this.menu.element.is(":visible")||k.preventDefault();else if(!d){var p=c.ui.keyCode;switch(k.keyCode){case p.PAGE_UP:this._move("previousPage",k);break;case p.PAGE_DOWN:this._move("nextPage",k);break;
case p.UP:this._keyEvent("previous",k);break;case p.DOWN:this._keyEvent("next",k)}}},input:function(k){e?(e=!1,k.preventDefault()):this._searchTimeout(k)},focus:function(){this.selectedItem=null;this.previous=this._value()},blur:function(k){this.cancelBlur?delete this.cancelBlur:(clearTimeout(this.searching),this.close(k),this._change(k))}});this._initSource();this.menu=c("<ul>").appendTo(this._appendTo()).menu({role:null}).hide().menu("instance");this._addClass(this.menu.element,"ui-autocomplete",
"ui-front");this._on(this.menu.element,{mousedown:function(k){k.preventDefault();this.cancelBlur=!0;this._delay(function(){delete this.cancelBlur;this.element[0]!==c.ui.safeActiveElement(this.document[0])&&this.element.trigger("focus")})},menufocus:function(k,p){if(this.isNewMenu&&(this.isNewMenu=!1,k.originalEvent&&/^mouse/.test(k.originalEvent.type))){this.menu.blur();this.document.one("mousemove",function(){c(k.target).trigger(k.originalEvent)});return}var r=p.item.data("ui-autocomplete-item");
!1!==this._trigger("focus",k,{item:r})&&k.originalEvent&&/^key/.test(k.originalEvent.type)&&this._value(r.value);(r=p.item.attr("aria-label")||r.value)&&c.trim(r).length&&(this.liveRegion.children().hide(),c("<div>").text(r).appendTo(this.liveRegion))},menuselect:function(k,p){var r=p.item.data("ui-autocomplete-item"),w=this.previous;this.element[0]!==c.ui.safeActiveElement(this.document[0])&&(this.element.trigger("focus"),this.previous=w,this._delay(function(){this.previous=w;this.selectedItem=r}));
...
function defaultAuto(c,m){c.autocomplete({source:function(q,a){$.getJSON({url:"https://widgets.ebscohost.com/prod/simplekey/autocomplete/autocomp.php",data:{q:encodeURIComponent(q.term)}}).done(function(d){d=JSON.parse(d);var e=[];d=$.map(d,function(h){return[h]})[2];for(var f=0;f<d.length;f++)e.push(d[f].term),a(e)}).fail(function(d,e,f){ga("send","event","eds autosuggest","error",f)})},select:function(q,a){a.item&&c.val(a.item.value);"quick"!==m?submitSearch(c.val()):c.closest("form").submit()}})}
...
$(function(){defaultAuto($("#dbpage-query"));$(".db-name").each(function(){for(var q=$(this),a=q.html().split(" "),d=/^Ahfs|^Apa$|^Dsm|^Ebsco|Cinahl|Cq|Eric|^Mas$|Medline|^Sage/,e=0;e<a.length;e++)d.test(a[e])&&(a[e]=a[e].toUpperCase());a=a.join(" ");q.html(a);-1<q.html().indexOf("Ebook Col")&&q.html(q.html().replace("Ebook","eBook"))});var c=$("#main-nav"),m=$("#nav-label");781>$(window).width()&&c.addClass("hidden");m.on("click",function(){c.toggleClass("hidden")});$(".category").each(function(){var q=
$(this);q.removeClass("active hidden");var a=q.find("li");a.hasClass("active")&&a.is(":visible")?q.addClass("active"):q.addClass("hidden")});checkZeros();$("dt").prev("dd").addClass("last-dd");$("#trial-dbs .db-desc").each(function(){var q=$(this),a=q.html(),d=$.trim(a).substring(0,60).split(" ").slice(0,-1).join(" ")+'... <button class="trial-desc-exp" type="button">more</button>';q.html(d);q.show();$(".trial-desc-exp").on("click",function(){$(this).parent().html(a)})})});
$("#dbpage-query").on("focus",function(){var c=$(this),m=$("#form-remainder");showSearch(m,c);$("html").on("click",function(q){$(q.target).parents("#multi-search").length||m.hasClass("hidden")||hideSearch(m,c)})});$(".alpha").each(function(){0===$(this).find("li").length&&$(this).addClass("hidden")});
...
$(".headnav a, #choose-library button").on("click",function(){var c=$(this).text().toLowerCase();setCookie({name:"homeLibrary",value:c,exp:30})});function getCookie(c){c+="=";for(var m=document.cookie.split(";"),q=0;q<m.length;q++){for(var a=m[q];" "==a.charAt(0);)a=a.substring(1);if(0===a.indexOf(c))return a.substring(c.length,a.length)}return""}
function homeLibEls(c){var m={arc:{cust:"amerriv",fod:"240535",gale:"sacr22807",singleCol:"a"},crc:{cust:"cosum",fod:"237206",gale:"sacr73031",singleCol:"c"},flc:{cust:"ns015092",fod:"237742",gale:"sacr88293",singleCol:"f"},scc:{cust:"sacram",fod:"106093",gale:"cclc_sac",singleCol:"s"}};$("#"+c+"-link").addClass("homelib");var q=function(d){d=d.split("-");var e=new Date;e.setFullYear(d[0],d[1]-1,d[2]);return e},a=new Date;$(".db-entry").each(function(){var d=$(this);d.data("college")&&-1===d.data("college").indexOf(m[c].singleCol)&&
d.hide();if(d.data("expiration")&&""!==d.data("expiration")){var e=q(d.data("expiration"));e.setHours(23,59,59);a>e&&d.hide()}d.data("startdate")&&""!==d.data("startdate")&&(e=q(d.data("startdate")),e.setHours(0),a<e?(e=d.find(".db-name"),e.hide().after(e.text()),d.find(".open-db-search").length&&d.find(".open-db-search").hide()):d.find(".start-note").hide())});0===$("#trial-dbs .db-entry:visible").length&&$("#trial-dbs").hide();$(".db-name:contains(OneSearch)").attr("href","https://caccl-lrccd.primo.exlibrisgroup.com/discovery/search?vid=01CACCL_LRCCD:"+
...
$(".open-db-search").on("click",function(){var c=$(this),m="";"yes"===getCookie("newWindowLinks")&&(m=' target="_blank" ');var q=c.nextAll("h3").find(".db-name"),a=q.text(),d=a.toLowerCase();d=d.replace(/ /g,"-");var e=c.nextAll("h3").find(".vendor").text().toLowerCase();e=e.replace(/[\(\):]/g,"");e=e.replace(/ /g,"-");var f=document.getElementById(d);$(".open-db-search").removeClass("hidden");var h=$(".search-one-db");if(f)c.addClass("hidden"),h.hide(),c.next(h).fadeIn(),document.getElementById(d).focus();
else{h.hide();f=q.attr("href");var k="";-1<f.indexOf("ehost")&&(k='<input type="hidden" name="ehost" value="'+f.split(/[= ]+/).pop()+'">');m="<form onsubmit=\"ga('send', 'event', 'quick search', 'submit', '"+a+'\');" class="search-one-db" method="post" action="search-db.php" '+m+'><label class="search-one-db-l" for="'+d+'">Search '+a+'</label><input name="query" id="'+d+'" type="text"><input type="hidden" name="vendor" value="'+e+'"><input type="hidden" name="db-name" value="'+d+'">'+k+'<input type="hidden" name="db-url" value="'+
encodeURIComponent(q.attr("href"))+'"><button class="search-btn" type="submit"><img height="16" width="16" src="search.png" alt="search"></button></form>';c.addClass("hidden");c.after(m);c.next(h).fadeIn();c=c.next(h).find(":text");c.focus();defaultAuto(c,"quick")}});$("#ga-opt-out").on("click",function(c){c.preventDefault();var m=screen.availHeight;c=.7*m;m*=.15;var q=screen.width/2-350,a=$(this).attr("href");window.open(a,"optOut","height="+c+", width=700, menubar=no, left="+q+", top="+m).focus()});
...
function removeProxy(){$(".db-name").each(function(){var m=$(this).attr("href");if(-1<m.indexOf("//0-")){var q=m.replace("//0-","//");-1<m.indexOf("https")&&(m=q.split(".edu/"),m[0]=m[0].replace(/\-/g,".").replace(/$/,".edu/"),q=m.join(""),console.log(q));q=q.replace(".lasiii.losrios.edu","");$(this).attr("href",q)}else-1<m.indexOf("ezproxy")&&(q=m.replace(/https:\/\/ezproxy\.losrios\.edu\/login\?(auth=custom&)?url=/,""),$(this).attr("href",q),$("#sso").hide())});var c=$("#proxy");$("#remove-proxy").attr("id",
...
function showNote(c){if("hide"!==getCookie("dbHideAlert")){var m=new Date,q=function(e){e=e.split("-");return new Date(e)},a=new Date(1970);c.start&&(a=q(c.start));var d=new Date(3E3);c.end&&(d=q(c.end));c=c.message||"";m>=a&&m<=d&&""!==c&&($('<p id="db-alert" role="alert" style="display:none;">'+c+'<button type="button" id="message-dismiss">Hide this message</button></p>').prependTo($("#main")).fadeIn(),$("#message-dismiss").on("click",function(){$("#db-alert").fadeOut();setCookie({name:"dbHideAlert",
value:"hide",exp:2})}))}};
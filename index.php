...
$dbCats = array('General', 'Art', 'Business', 'Communication', 'Controversial Topics', 'Criminal Justice &amp; Law', 'Ebooks', 'Education', 'Encyclopedias, Dictionaries, Reference', 'Environmental Science', 'Health &amp; Medicine', 'History', 'Literature', 'Music', 'News', 'Philosophy &amp; Religion', 'Political Science', 'Psychology', 'Sociology', 'Theatre &amp; Performing Arts', 'Video' );
...
elseif ((isset($alpha)) && (!isset($format)) ) {
    if (($alpha === 'all') || (empty($alpha))) {
     ...
     if ((isset($format)) || ((isset($query)) && (!(empty($query))))){
        ...
        echo 'It looked like you were searching for a database so we searched this page for it. If that&apos;s not what you wanted, <a href="http://0-search.ebscohost.com.lasiii.losrios.edu/login.aspx?authtype=ip&amp;groupid=main&amp;profile=eds&amp;direct=true&amp;site=eds-live&amp;bquery=' . $query . '">search for <span id="check-search-query">' . $query . '</span> in OneSearch</a>.';
      echo "</div>\n";
     }
    }
